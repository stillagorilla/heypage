...
(function ($) {
  ...
  jQuery.fn.Am2_SimpleSlider = function () {
    //popup div
    $div = $('<div class="product-gallery-popup"> <div class="popup-overlay"></div> <div class="product-popup-content"> <div class="product-image"> <img id="nav-btn-prev" src="" alt="" /> <div class="gallery-nav-btns"> <a id="nav-btn-next" class="nav-btn next" ></a> <a id="nav-btn-prev" class="nav-btn prev" ></a></div> </div><div class="product-information"></div><a role="button" class="times p-2"><i class="fa fa-times"></i></a></div></div>').appendTo('body');

    //on image click   
    $(this).click(function () {
        $('.product-gallery-popup').fadeIn(500);
        $('body').css({ 'overflow': 'hidden' });
        //var myContent = $(this).closest('.card-body').find('.content').html();
        var myCommentBox = $(this).closest('.card-body').find('.add-comment').html();

        $('.product-popup-content .product-image img').attr('src', $(this).find('img').attr('src'));
        //$('.product-popup-content .product-information').append(myCommentBox);
        
        $Current = $(this);
        $PreviousElm = $(this).prev();
        $nextElm = $(this).next();
        if ($PreviousElm.length === 0) { $('.nav-btn.prev').css({ 'display': 'none' }); }
        else { $('.nav-btn.prev').css({ 'display': 'flex' }); }
        if ($nextElm.length === 0) { $('.nav-btn.next').css({ 'display': 'none' }); }
        else { $('.nav-btn.next').css({ 'display': 'flex' }); }
    });
    //on Next click
    $('.next').click(function () {
        $NewCurrent = $nextElm;
    ...
    $('.prev').click(function () {
      ...
      //Close Popup
      $('.times,.popup-overlay').click(function () {
          $('.product-gallery-popup').fadeOut(500);
          $('body').css({ 'overflow': 'initial' });
        //  $('.product-information .comments21').empty();
